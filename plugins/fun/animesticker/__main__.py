...
async def anime_sticker(message: Message):
    ...
    if "-ggl" in message.flags:
        try:
            stickers = await userge.get_inline_bot_results(
                "stickerizerbot", f"#12{get_emoji_regexp().sub(u'', text)}"
            )
    ...
    try:
        stickers = await userge.get_inline_bot_results(
            "stickerizerbot", f"#{animus}{get_emoji_regexp().sub(u'', text)}"
        )
        saved = await userge.send_inline_bot_result(
            chat_id="me", query_id=stickers.query_id, result_id=stickers.results[0].id
        )
