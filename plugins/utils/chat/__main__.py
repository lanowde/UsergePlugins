...
def mention_html(user_id, name):
    return '<a href="tg://user?id={}">{}</a>'.format(  # pylint: disable=consider-using-f-string
...
async def tagall_(message: Message):
    ...
    try:
        async for members in message.client.get_chat_members(
            c_id, filter=enums.ChatMembersFilter.RECENT
        ):
...
async def view_chat(message: Message):
    ...
    if "-title" in message.flags:
        ...
        if not chat.description:
            ...
            await message.edit("```\nchecking, Wait plox !...```", del_in=3)
            await message.edit(
                f"<code>{chat.description}</code>", parse_mode=enums.ParseMode.HTML
            )
    else:
        if not chat.photo:
