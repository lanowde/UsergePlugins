...
async def joinvc(msg: Message):
    ...
    if join_as:
        ...
        if raw_id not in [
            getattr(peers, "user_id", None) or getattr(peers, "channel_id", None)
...
@userge.on_cmd("leavevc", about={"header": "Leave Video-Chat", "usage": "{tr}leavevc"})
...
@userge.on_cmd(
    ...
    about={
        ...
        "flags": {"-v": "Stream as video.", "-q": "Quality of video stream (1-100)"},
    },
...
@userge.on_cmd(
    "current",
    about={"header": "View Current playing Song.", "usage": "{tr}current"},
    trigger=config.PUBLIC_TRIGGER,
    check_client=True,
    filter_me=False,
    allow_bots=False,
...
@userge.on_cmd(
    "volume", about={"header": "Set volume", "usage": "{tr}volume\n{tr}volume 69"}
)
@vc_chat
...
@userge.on_cmd(
    ...
    about={
        ...
        "flags": {"-to": "To jump to a specific timestamp"},
        "examples": [
...
async def resume_music(msg: Message):
    ...
    CURRENT_SONG["start"] = CURRENT_SONG["start"] + time.time() - CURRENT_SONG["pause"]
...
@userge.on_cmd(
    "shuffle",
    about={"header": "Shuffle songs in queue.", "usage": "{tr}shuffle"},
    trigger=config.PUBLIC_TRIGGER,
    check_client=True,
    filter_me=False,
    allow_bots=False,
...
@userge.on_cmd(
    "stopvc", about={"header": "Stop vc and clear Queue.", "usage": "{tr}stopvc"}
)
@vc_chat
...
async def _participants_change_handler(_: PyTgCalls, update: Update):
    if isinstance(update, JoinedGroupCallParticipant):
        ...
        isinstance(update, LeftGroupCallParticipant)
...
if userge.has_bot:
    ...
    userge.bot.add_handler(
        CallbackQueryHandler(
            vc_control_callback, filters.regex("(player|seek|rewind|replay)")
        )
    )
