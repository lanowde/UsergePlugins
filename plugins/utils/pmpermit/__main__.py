...
@userge.on_cmd(
    "listpm", about={"header": "List all Allowed PM's", "usage": "{tr}listpm"}
)
...
@userge.on_cmd(
    ...
    about={
        ...
        "usage": "{tr}ipmmsg [text | reply to text msg]",
    },
    allow_channels=False,
)
...
async def uninvitedPmHandler(message: Message):
    ...
    if message.from_user.id in pmCounter:
        if pmCounter[message.from_user.id] > 3:
            ...
            await message.reply(blocked_message.format_map(SafeDict(**user_dict)))
        ...
        if userge.has_bot and pmpermit.Dynamic.IS_INLINE:
            try:
                ...
                await userge.send_inline_bot_result(
                    message.chat.id, query_id=k.query_id, result_id=k.results[0].id
                )
...
@userge.on_filters(
    ~allowAllFilter & filters.outgoing & filters.private & ~pmpermit.ALLOWED_CHATS,
    allow_via_bot=False,
)
...
if userge.has_bot:
    ...
    async def pmpermit_inline_query_handler(_, query: InlineQuery):
        ...
        if pm_inline_msg:
            ...
            text = (
                f"Hello, welcome to **{owner.first_name}** Dm.\n\nWhat you want to do ?"
            )
        buttons = [
            [
