...
async def _get_banned_rights(message: Message) -> ChatBannedRights:
    ...
    if isinstance(peer, InputPeerChannel):
        return (
            (await message.client.invoke(GetFullChannel(channel=peer)))
            .chats[0]
            .default_banned_rights
        )
    return (
        (await message.client.invoke(GetFullChat(chat_id=peer.chat_id)))
        .chats[0]
        .default_banned_rights
    )


async def _get_new_rights(
    message: Message, lock_type: str, should_lock: bool
) -> Tuple[ChatBannedRights, str]:
...
async def _edit_ban_rights(
    message: Message, rights: Optional[ChatBannedRights] = None
) -> None:
    if rights is None:
...
async def lock_perm(message: Message):
    ...
    try:
        ...
        await message.edit(
            f"Nothing was changed, since {perm} is already locked.", del_in=5
        )
    except Exception as e_f:
        await message.edit(
            r"`i don't have permission to do that ＞︿＜`\n\n" f"**ERROR:** `{e_f}`",
...
async def unlock_perm(message: Message):
    ...
    try:
        ...
        await message.edit(
            f"Nothing was changed, since {perm} is not locked here.", del_in=5
        )
    except Exception as e_f:
        await message.edit(
            r"`i don't have permission to do that ＞︿＜`\n\n" f"**ERROR:** `{e_f}`",
...
async def view_perm(message: Message):
    ...
    permission_view_str += (
        f"<b>📩 Send Messages:</b> {_emojify(ban_rights.send_messages)}\n"
    )
    permission_view_str += f"<b>🎭 Send Media:</b> {_emojify(ban_rights.send_media)}\n"
    permission_view_str += (
        f"<b>🎴 Send Stickers:</b> {_emojify(ban_rights.send_stickers)}\n"
    )
    permission_view_str += (
        f"<b>🎲 Send Animations:</b> {_emojify(ban_rights.send_gifs)}\n"
    )
    permission_view_str += (
        f"<b>🎮 Can Play Games:</b> {_emojify(ban_rights.send_games)}\n"
    )
    permission_view_str += (
        f"<b>🤖 Can Use Inline Bots:</b> {_emojify(ban_rights.send_inline)}\n"
    )
    permission_view_str += (
        f"<b>🌐 Webpage Preview:</b> {_emojify(ban_rights.embed_links)}\n"
    )
    ...
    permission_view_str += (
        f"<b>👥 Invite Users:</b> {_emojify(ban_rights.invite_users)}\n"
    )
    permission_view_str += (
        f"<b>📌 Pin Messages:</b> {_emojify(ban_rights.pin_messages)}\n"
    )
