...
def _get_msg_and_log(
    chat: Chat, user: User, reason: str, fed: Optional[str]
) -> Tuple[str, str]:
    user_id = user.id
...
class GBanHandler(AbstractHandler):
    ...
    async def on_handle(self, chat: Chat, user: User, data) -> None:
        ...
        await GBAN_USER_BASE.update_one(
            {"user_id": user.id, "firstname": user.first_name},
...
async def antispam_(message: Message):
    ...
    text = (
        f"`antispam {'enabled' if antispam.Dynamic.ANTISPAM_SENTRY else 'disabled'} !`"
    )
