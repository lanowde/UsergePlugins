...
async def account_login(client, url):
    data = {"email": parser.APPDRIVE_EMAIL, "password": parser.APPDRIVE_PASS}
...
async def appdrive_dl(url):
    ...
    data = {"key": key, "action": "original", "type": 1}
    ...
    while data["type"] <= 3:
        try:
            response = (
                await pool.run_in_thread(client.post)(
                    url, data=gen_payload(data), headers=headers
                )
            ).json()
            break
    ...
    if "url" in response:
        ...
        info_parsed["error_message"] = response.get(
...
async def gdtot(message: Message):
    ...
    if not args:
        ...
        try:
            ...
            title = soup.find(
                "h5", {"class": lambda x: x and "modal-title" not in x}
            ).text
