...
FF_MPEG_DOWN_LOAD_MEDIA_PATH = Path(
    f"{config.Dynamic.DOWN_PATH.rstrip('/')}/userge.media.ffmpeg"
)
...
@userge.on_cmd(
    ...
    about={
        ...
        "flags": {"-b": "Custom bitrate", "-d": "Delete media after process done"},
        "usage": "{tr}x265 [file / folder path | direct link | reply to telegram file]",
        "examples": ["{tr}x265 link", "{tr}x265 path"],
    },
    check_downpath=True,
)
...
@userge.on_cmd(
    ...
    about={
        ...
        "examples": ["{tr}v2a link", "{tr}v2a path"],
    },
    check_downpath=True,
)
...
@userge.on_cmd(
    ...
    about={
        ...
        "flags": {
            "-q": "Video Quality 144/240/360/480/720/etc",
            "-b": "Custom bitrate",
        ...
        "examples": ["{tr}vscale -q360 link", "{tr}vscale -d -q360 path"],
    },
    check_downpath=True,
)
async def scale_video(message: Message):
    ...
    try:
        await run(
            ...
            .output(video_file, vcodec=encoder, crf=custom_bitrate, preset="ultrafast")
        )
    except ffmpeg.Error as e:
        await message.err(f"{e.stderr}")
    else:
...
@userge.on_cmd(
    ...
    about={
        ...
        "examples": ["{tr}vth link", "{tr}vth path"],
    },
    check_downpath=True,
)
async def video_thumbnail(message: Message):
    ...
    try:
        await run(
            ffmpeg.input(dl_loc, ss="00:01:00")
            .filter("scale", 720, -1)
...
async def video_trim(message: Message):
    ...
    start_time = (
        match.group(1) if match.group(1) and match.group(1) != end_time else "00:00:00"
    )
...
@userge.on_cmd(
    ...
    about={
        ...
        "examples": ["{tr}vcompress 70 link", "{tr}vcompress 50 path"],
    },
    check_downpath=True,
)
...
@userge.on_cmd(
    ...
    about={
        ...
        "examples": ["{tr}minfo link", "{tr}minfo path"],
    },
    check_downpath=True,
)
